...
function get_input(mode: InputMode) {
  switch (mode) {
    case InputMode.File:
...
class Area {
  ...
  static new(line: string): Result<Area> {
    ...
    return {
      payload: new Area(Vector.new(startx, starty), Vector.new(endx, endy)),
    };
  }
...
function main() {
  let input = get_input(InputMode.File);
